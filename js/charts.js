...
export async function renderGdpGapChart() {
    ...
    try {
        ...
        gdpGapChart = new Chart(ctx, { type: 'bar', data: { labels: labels, datasets: [{ label: 'GDP 갭 (%)', data: gdpGapDataValues, backgroundColor: gdpGapDataValues.map(v => v >= 0 ? 'rgba(220, 53, 69, 0.7)' : 'rgba(0, 86, 179, 0.7)') }] }, options: { responsive: true, maintainAspectRatio: false, scales: { x: { ticks: { callback: (v, i) => (parseInt(labels[i].substring(0, 4)) % 5 === 0 && labels[i].substring(5,7) === '01') ? labels[i].substring(0, 4) : '', autoSkip: false, maxRotation: 0 } }, y: { title: { display: true, text: 'GDP 갭 (%)' } } }, plugins: { legend: { display: false } } } });
        
        return labels.map((date, index) => ({ date: date, value: gdpGapDataValues[index] }));
    } catch(error) {
...
export async function renderGdpConsumptionChart() {
    ...
    try {
        const [gdpObs, pceObs, usrecObs] = await Promise.all([ fetchFredData('GDPC1', 220, 'desc'), fetchFredData('PCEC', 220, 'desc'), fetchFredData('USRECQ', 220, 'desc') ]);
        ...
        for (let i = 4; i < uniqueDates.length; i++) {
            ...
            if ([currentGdp, prevGdp, currentPce, prevPce].every(v => !isNaN(v))) {
                chartData.push({ date: currentDate, gdpGrowth: ((currentGdp / prevGdp) - 1) * 100, pceGrowth: ((currentPce / prevPce) - 1) * 100, isRecession: usrecMap.get(currentDate) || false });
            }
        }
        if (chartData.length === 0) throw new Error("GDP/소비 데이터 가공에 실패했습니다.");
        
        const labels = chartData.map(d => d.date);
        const recessionPeriods = { '1973-11-01': '오일 쇼크', '1980-01-01': '더블 딥 침체', '1990-07-01': '걸프전 침체', '2001-03-01': 'IT 버블', '2007-12-01': '금융위기', '2020-02-01': '팬데믹' };
        const recessionAnnotations = []; let startRecession = null, recessionStartDate = null;
        chartData.forEach((d, index) => {
            if (d.isRecession && startRecession === null) { startRecession = index; recessionStartDate = d.date; } 
            else if (!d.isRecession && startRecession !== null) {
                const labelKey = Object.keys(recessionPeriods).find(key => new Date(key) >= new Date(recessionStartDate) && new Date(key) < new Date(d.date));
                const annotation = { type: 'box', xMin: startRecession, xMax: index, backgroundColor: 'rgba(220, 53, 69, 0.1)', borderColor: 'transparent' };
                if (labelKey) { annotation.label = { content: recessionPeriods[labelKey], display: true, position: 'start', yAdjust: 10, font: { size: 11 }, color: 'rgba(220, 53, 69, 0.8)' }; }
                recessionAnnotations.push(annotation); startRecession = null; recessionStartDate = null;
            }
        });
        gdpConsumptionChart = new Chart(ctx, { type: 'line', data: { labels, datasets: [ { label: '실질 GDP 성장률 (%)', data: chartData.map(d => d.gdpGrowth), borderColor: '#28a745', borderWidth: 2, pointRadius: 0 }, { label: '실질 PCE(소비) 성장률 (%)', data: chartData.map(d => d.pceGrowth), borderColor: '#0056b3', borderWidth: 2, pointRadius: 0 } ] }, options: { responsive: true, maintainAspectRatio: false, interaction: { mode: 'index', intersect: false }, scales: { x: { ticks: { callback: (v, i) => (parseInt(labels[i].substring(0, 4)) % 5 === 0 && labels[i].substring(5,7) === '01') ? labels[i].substring(0, 4) : '', autoSkip: false, maxRotation: 0 } }, y: { title: { display: true, text: '성장률 (%)' } } }, plugins: { legend: { position: 'top' }, annotation: { annotations: recessionAnnotations } } } });
...
export async function renderMarshallKChart() {
    ...
    try {
        const [gdpSeries, m2Series, rateSeries] = await Promise.all([ fetchFredData('GDP', 2000, 'desc'), fetchFredData('M2SL', 5000, 'desc'), fetchFredData('DGS10', 15000, 'desc') ]);
        ...
        const crisisAnnotations = [ { date: '2001-03-01', label: 'IT 버블' }, { date: '2007-12-01', label: '금융위기' }, { date: '2020-02-01', label: '팬데믹' } ].map(c => { const index = chartData.findIndex(d => d.date >= new Date(c.date)); return index === -1 ? null : { type: 'line', scaleID: 'x', value: index, borderColor: 'rgba(220, 53, 69, 0.7)', borderWidth: 2, borderDash: [6, 6], label: { content: c.label, display: true, position: 'start', yAdjust: -5, font: { size: 12 }, color: 'white', backgroundColor: 'rgba(220, 53, 69, 0.8)' } }; }).filter(Boolean);
        marshallKChart = new Chart(ctx, { type: 'line', data: { labels: chartData.map(d => d.label), datasets: [ { label: '국채 10년 (%)', data: chartData.map(d => d.interestRate), borderColor: '#0056b3', yAxisID: 'y', borderWidth: 2, pointRadius: 0 }, { label: '마샬케이', data: chartData.map(d => d.marshallK), borderColor: '#212529', yAxisID: 'y1', borderWidth: 2, pointRadius: 0 } ] }, options: { responsive: true, maintainAspectRatio: false, scales: { x: { ticks: { callback: (v, i) => (chartData[i].year % 4 === 0 && chartData[i].label.endsWith('Q1')) ? chartData[i].year : '', autoSkip: false, maxRotation: 0 } }, y: { position: 'left', title: { display: true, text: '금리 (%)' } }, y1: { position: 'right', title: { display: true, text: '마샬케이' }, grid: { drawOnChartArea: false } } }, plugins: { legend: { position: 'top' }, annotation: { annotations: crisisAnnotations, clip: false } } } });

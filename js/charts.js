...
import { analyzeMarshallKTrend, analyzeGdpGap, analyzeGdpConsumption } from './js/analysis.js';
...
export async function renderGdpConsumptionChart(resultsObject) {
    ...
    try {
        ...
        for (let i = 4; i < uniqueDates.length; i++) {
            const currentDate = uniqueDates[i]; const previousDate = uniqueDates[i - 4];
            const currentGdp = gdpMap.get(currentDate); const prevGdp = gdpMap.get(previousDate);
            const currentPce = pceMap.get(currentDate); const prevPce = pceMap.get(previousDate);
        ...
        const recessionAnnotations = []; let startRecession = null; let recessionStartDate = null;
        chartData.forEach((d, index) => {
            if (d.isRecession && startRecession === null) { startRecession = index; recessionStartDate = d.date; } 
                ...
                if (labelKey) { annotation.label = { content: recessionPeriods[labelKey], display: true, position: 'start', yAdjust: 10, font: { size: 11, weight: 'bold' }, color: 'rgba(220, 53, 69, 0.8)' }; }
        ...
        gdpConsumptionChart = new Chart(ctx, {
            type: 'line',
            data: { labels: labels, datasets: [ { label: '실질 GDP 성장률 (%)', data: chartData.map(d => d.gdpGrowth), borderColor: '#28a745', borderWidth: 2, pointRadius: 0, tension: 0.1 }, { label: '실질 PCE(소비) 성장률 (%)', data: chartData.map(d => d.pceGrowth), borderColor: '#0056b3', borderWidth: 2, pointRadius: 0, tension: 0.1 } ] },
            options: {
...
export async function renderMarshallKChart(resultsObject) {
    ...
    try {
        ...
        rateSeries.filter(p => p.value !== '.').forEach(p => { const monthKey = p.date.substring(0, 7); if (!rateMonthlyAvg.has(monthKey)) rateMonthlyAvg.set(monthKey, []); rateMonthlyAvg.get(monthKey).push(parseFloat(p.value)); });
        ...
        gdpMap.forEach((gdpValue, gdpDate) => {
            const date = new Date(gdpDate); const year = date.getFullYear(); const quarter = Math.floor(date.getMonth() / 3) + 1;
            const quarterMonths = Array.from({length: 3}, (_, i) => `${year}-${String((quarter - 1) * 3 + i + 1).padStart(2, '0')}`);
            const m2Values = quarterMonths.map(m => m2Map.get(m)).filter(v => v); const rateValues = quarterMonths.map(m => rateMap.get(m)).filter(v => v);
            if (m2Values.length > 0 && rateValues.length > 0) {
                const avgM2 = m2Values.reduce((a, b) => a + b, 0) / m2Values.length; const avgRate = rateValues.reduce((a, b) => a + b, 0) / rateValues.length;
                chartData.push({ label: `${year} Q${quarter}`, year: year, marshallK: (avgM2 / gdpValue), interestRate: avgRate, date: date });
            }
        });
        ...
        const crisisAnnotations = [ { date: '2001-03-01', label: 'IT 버블' }, { date: '2007-12-01', label: '금융위기' }, { date: '2020-02-01', label: '팬데믹' } ].map(c => {
            const index = chartData.findIndex(d => new Date(d.date) >= new Date(c.date)); if (index === -1) return null;
            return { type: 'line', scaleID: 'x', value: index, borderColor: 'rgba(220, 53, 69, 0.7)', borderWidth: 2, borderDash: [6, 6], label: { content: c.label, display: true, position: 'start', yAdjust: -5, font: { size: 12, weight: 'bold' }, backgroundColor: 'rgba(220, 53, 69, 0.8)', color: 'white', padding: 4, borderRadius: 4 } };
        }).filter(a => a !== null);

        marshallKChart = new Chart(ctx, {
            type: 'line',
            data: { labels: chartData.map(d => d.label), datasets: [ { label: '국채 10년 (%)', data: chartData.map(d => d.interestRate), borderColor: '#0056b3', yAxisID: 'y', borderWidth: 2, pointRadius: 0 }, { label: '마샬케이', data: chartData.map(d => d.marshallK), borderColor: '#212529', yAxisID: 'y1', borderWidth: 2, pointRadius: 0 } ] },
            options: {
                responsive: true, maintainAspectRatio: false, interaction: { mode: 'index', intersect: false },
                scales: {
                ...
                plugins: { legend: { position: 'top' }, annotation: { annotations: crisisAnnotations, clip: false } }
            }
        });
    ...
    try {
        ...
        if (historicalData && historicalData.length > 0) {
            ...
            indicatorChart = new Chart(ctx, {
                type: 'line',
                data: { labels: historicalData.map(d => d.date), datasets: [{ label: details.title.replace(/[\u{1F1E6}-\u{1F1FF}]/gu, '').trim(), data: historicalData.map(d => d.value), borderColor: '#0056b3', borderWidth: 2, pointRadius: 1, tension: 0.1 }] },
                options: { responsive: true, maintainAspectRatio: false }
            });
        }
    } catch(error) {
